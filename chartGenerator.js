...
function setAction(text, x, y, callback = () => { }) {
    ...
    const width = setText(action, { x: x, y: y, fontSize: 64, color: actionColors[text] })

    loadImage(`img/${arrow}.svg`).then((image) => {
...
module.exports.generateSignalImage = async function (robot, signal) {
    ...
    loadImage('img/background.png').then((image) => {
        ...
        setText(signalDate, { x: canvas.width - 60, y: 40, textAlign: "end", color: "#6987B9" })
        ...
        setText(orderType, { x: canvas.width - 80, y: 468, fontSize: 64, textAlign: "end" })
        ...
        setAction(signal.action, 280, 468, () => {
            ...
            base64Img.img(canvas.toDataURL(), 'dest', fileName, function (err, filepath) { });
        })
    })
}